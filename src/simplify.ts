...
export function vw(
    ...
    */
    ...
    if (options.rate) {
        if (options.threshold < 0 || options.threshold >= 1)
            throw new InvalidSimplifyError("when rate is true, threshold must be between 0 and 1.");
    } else {
        if (options.threshold < 0)
            throw new InvalidSimplifyError("threshold must be positive number.");
    }
    ...
    const threshold = options.rate ? calcArea(linearRing) * options.threshold : options.threshold;
    ...
    const candidates = linearRing.map((point, i) => {
        ...
        return {
            score: calcArea([linearRing[i - 1], point, linearRing[i + 1], linearRing[i - 1]]),
            index: i
        }
    })
